<template>
    <div class="official">
        <div class="scroll">
            <h3 style="border-top: none; padding-top: 0;">Ressource</h3>
            <p>
            ...
            <p>En vertu de l'article 6 de la loi n° 2004-575 du 21 juin 2004 pour la confiance dans l'économie
                numérique, il
                est précisé aux utilisateurs du site <a :href="'https://' + website">www.{{ website }}</a>
                l'identité
                des
                différents intervenants dans le
                cadre de sa réalisation et de son suivi :
                <br><br>
                Propriétaire : {{ owner }} – {{ ownerAddress }}
                <br><br>
            ...
            <p>L'utilisation du site <a :href="'https://' + website">www.{{ website }}</a> implique l'acceptation
                pleine et
                entière des conditions
                générales d'utilisation ci-après décrites. Ces conditions d'utilisation sont susceptibles d'être
                modifiées
                ou
                complétées à tout moment, les utilisateurs du site <a :href="'https://' + website">www.{{ website
                    }}</a>
                ...
                maintenance
                technique peut être toutefois décidée par <a :href="'https://' + website">www.{{ website }}</a>,
                qui
                s'efforcera alors de communiquer
                préalablement aux utilisateurs les dates et heures de l'intervention.
                <br><br>
                Le site <a :href="'https://' + website">www.{{ website }}</a> est mis à jour régulièrement par
                {{ owner }}. De la même façon, les
            ...
            <p>Le site <a :href="'https://' + website">www.{{ website }}</a> a pour objet de fournir une
                information
                concernant l'ensemble des activités de
                la société.
                <br><br>
                {{ owner }} s'efforce de fournir sur le site <a :href="'https://' + website">www.{{ website
                    }}</a>
                des
                ...
                informations.
                <br><br>
                Tous les informations indiquées sur le site <a :href="'https://' + website">www.{{ website }}</a>
                sont
                données à titre indicatif, et sont
                susceptibles d'évoluer. Par ailleurs, les renseignements figurant sur le site <a
                    :href="'https://' + website">www.{{ website }}</a> ne sont
            ...
            <p>{{ owner }} est propriétaire des droits de propriété intellectuelle ou détient les droits d'usage
                sur
                tous
            ...
            <p>{{ owner }} ne pourra être tenu responsable des dommages directs et indirects causés au matériel de
                l'utilisateur, lors de l'accès au site <a :href="'https://' + website">www.{{ website }}</a>, et
                résultant
                ...
                incompatibilité
                <br><br>
                {{ owner }} ne pourra également être tenu responsable des dommages indirects (tels par exemple
                qu'une
                perte de marché ou perte d'une chance) consécutifs à l'utilisation du site <a
                    :href="'https://' + website">www.{{ website }}</a>.
                <br><br>
                Des espaces interactifs (possibilité de poser des questions dans l'espace contact) sont à la
                disposition
                des
                utilisateurs. {{ owner }} se réserve le droit de supprimer, sans mise en demeure préalable, tout
                contenu
                déposé dans cet espace qui contreviendrait à la législation applicable en France, en particulier aux
                dispositions relatives à la protection des données. Le cas échéant, {{ owner }} se réserve
                également
                la
            ...
            <p>En France, les données personnelles sont notamment protégées par la loi n° 78-87 du 6 janvier 1978,
                ...
                1995.
                <br><br>
                À l'occasion de l'utilisation du site <a :href="'https://' + website">www.{{ website }}</a>,
                peuvent
                êtres
                recueillies : l'URL des liens par
                l'intermédiaire desquels l'utilisateur a accédé au site <a :href="'https://' + website">www.{{
                    website }}</a>, le fournisseur d'accès de
                l'utilisateur, l'adresse de protocole Internet (IP) de l'utilisateur.
                <br><br>
                En tout état de cause {{ owner }} ne collecte des informations personnelles relatives à
                l'utilisateur
                que
                pour le besoin de certains services proposés par le site <a :href="'https://' + website">www.{{
                    website }}</a>. L'utilisateur fournit ces
                informations en toute connaissance de cause, notamment lorsqu'il procède par lui-même à leur saisie.
                Il
                est
                alors précisé à l'utilisateur du site <a :href="'https://' + website">www.{{ website }}</a>
                l'obligation ou
                ...
                <br><br>
                Aucune information personnelle de l'utilisateur du site <a :href="'https://' + website">www.{{
                    website }}</a> n'est publiée à l'insu de
                l'utilisateur, échangée, transférée, cédée ou vendue sur un support quelconque à des tiers. Seule
                l'hypothèse du
                rachat de {{ owner }} et de ses droits permettrait la transmission des dites informations à
                l'éventuel
                acquéreur qui serait à son tour tenu de la même obligation de conservation . et de modification des
                données
                vis
                à vis de l'utilisateur du site <a :href="'https://' + website">www.{{ website }}</a>.
                <br><br>
            ...
            <p>Le site <a :href="'https://' + website">www.{{ website }}</a> contient un certain nombre de liens
                hypertextes vers d'autres sites, mis en
                place avec l'autorisation de {{ owner }}. Cependant, {{ owner }} n'a pas la possibilité de
                vérifier
                le contenu des sites ainsi visités, et n'assumera en conséquence aucune responsabilité de ce fait.
                <br><br>
                La navigation sur le site <a :href="'https://' + website">www.{{ website }}</a> ne nécessite pas
                l'installation de cookie(s) sur
                l'ordinateur de l'utilisateur.
            </p>

            <!-- La navigation sur le site <a :href="'https://' + website">www.{{ website }}</a> est susceptible de provoquer l'installation de cookie(s) sur
            ...
            <p>Tout litige en relation avec l'utilisation du site <a :href="'https://' + website">www.{{ website
                    }}</a>
                est
